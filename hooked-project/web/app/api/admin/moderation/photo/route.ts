...
import { updatePhotoModerationStatus } from "@/lib/server/user-store";
...
export async function POST(request: Request) {
  ...
  let body: { accountId?: string; status?: "none" | "pending" | "approved" | "rejected" };
  try {
  ...
  const result = await updatePhotoModerationStatus({
  ...
  await writeAuditLog({
    action: "admin.moderation.photo.update",
    actor: "admin",
